...
//
    ...
    private func makeParameterClassId(_ classID: Int) -> Parameters{
        return ["classID": classID]
    }
    
    func setMypageClassDetail(classId: Int, completion: @escaping (NetworkResult<Any>) -> Void) {
        
        let header: HTTPHeaders = ["jwt": UserDefaults.standard.object(forKey: "token") as? String ?? " "]
    

        let dataRequest = Alamofire.request(APIConstants.lectureURL + "/" + "\(classId)", method: .get, parameters: nil, encoding: JSONEncoding.default, headers: header)
        ...
        dataRequest.responseData { dataResponse in
            ...
            case .success :
                guard let statusCode = dataResponse.response?.statusCode else {return}
                guard let value = dataResponse.result.value else {return}
                let networkResult = self.judgeClassDetail(by: statusCode,value)
                completion(networkResult)
            case .failure : completion(.networkFail)
            }
        }
    ...
    private func judgeClassDetail(by statusCode: Int, _ data: Data) -> NetworkResult<Any> {
        switch statusCode {
            case 200:
                return isClassDetailData(by: data)
            case 400: return .pathErr
            case 500: return .serverErr
            default: return .networkFail
        }
    ...
    private func isClassDetailData(by data:Data) -> NetworkResult<Any> {
        let decoder = JSONDecoder()
        guard let decodedData = try? decoder.decode(MyClassDetailData.self, from: data)
            else {return .pathErr}
